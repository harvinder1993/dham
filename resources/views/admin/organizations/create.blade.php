                ...
                <div class="card-body">
                  <form method="POST" action="{{ route('admin.organization.store') }}">
                    ...
                    <div class="input-group input-group-outline mb-3">
                      ...
                      <input type="text" name="estd" id="estd" class="form-control" value="{{ old('estd') }}">
                        @error('estd')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="input-group input-group-outline mb-3 is-filled">
                      <label class="form-label">Helping Center</label>
                        <select class="form-control" id="helping_center" name="helping_center">
                          <option value="option1">Select Center</option>
                          @foreach($helping_centers as $helping_center)
                            <option value="{{ $helping_center->id }}">{{ $helping_center->name }}</option>
                          @endforeach 
                        </select>
